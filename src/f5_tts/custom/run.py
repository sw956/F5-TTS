...
#
# extracted from `infer_cli.py`
#
def synth(text: str, relative_output_folder: str, file_name: str, ref_audio_file: str, ref_audio_text: str, tts_model, vocoder, vocoder_name, target_sample_rate, target_rms, cross_fade_duration, nfe_step, cfg_strength, sway_sampling_coef, speed, fix_duration, device):
    DEFAULT_VOICE = "main"
    voices = {
        "main": {
    ...
    if generated_audio_segments:
        ...
        output_folder = importlib.resources.files("f5_tts").joinpath(f"{relative_output_folder}")
        if not os.path.exists(str(output_folder)):
            os.makedirs(str(output_folder))

        wav_file = str(output_folder.joinpath(file_name))
...
if __name__ == "__main__":
    ...
    synth_start_time_s = time.time()
    synth(
        text                   = TEXT_TO_SYNTHESIZE,
        relative_output_folder = OUTPUT_DIR,
    ...
    synth_end_time_s = time.time()

    synth_delta_s = synth_end_time_s - synth_start_time_s
    
    print(f"------------- DONE -------------    ( {synth_delta_s:.1f} seconds )")